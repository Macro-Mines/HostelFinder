...
function loadOwnerHostels(ownerUID) {
  ...
  db.collection("ownerHostelsSubmission")
    .where("ownerUID", "==", ownerUID)
    .get()
    .then((snapshot) => {
      if (snapshot.empty) {
        hostelList.innerHTML =
          '<div style="color:#888;font-size:1.07rem;">No hostels listed yet. Click "List New Hostel" to add one!</div>';
        return;
      }
      hostelList.innerHTML = "";
      snapshot.forEach((doc) => {
        ...
        hostelList.innerHTML += `
                    <div class="hostel-card">
                        ...
                        <div class="hostel-meta"><b>Location:</b> ${
                          hostel.location
                        }</div>
                        <div class="hostel-meta"><b>Gender:</b> ${
                          hostel.gender || "N/A"
                        }</div>
                        <div class="hostel-meta"><b>Distance:</b> ${
                          hostel.distance
                        } km</div>
                        
                        <div class="hostel-meta"><b>Min-Rent:</b> ${
                          hostel.minRent
                        }  | <b>Max-Rent:</b> ${hostel.maxRent}</div>
                        <div class="hostel-meta"><b>Contact:</b> ${
                          hostel.ownerContactName || ""
                        } (${hostel.ownerPhone || ""})</div>
                        <div class="hostel-meta"><b>Description:</b> ${
                          hostel.description
                        }</div>
                        
                        <div class="hostel-actions">
                            <button class="hostel-btn edit" onclick="editHostel('${
                              doc.id
                            }')">Edit</button>
                            <button class="hostel-btn delete" onclick="deleteHostel('${
                              doc.id
                            }', '${hostel.name}')">Delete</button>
                        </div>
                    </div>
                `;
      });
