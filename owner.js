...
form.onsubmit = async function (e) {
  ...
  const ownerContactName = document
    .getElementById("owner-contact-name")
    .value.trim();
  ...
  const roomCapacity = Array.from(
    document.querySelectorAll(".owner-room-capacity-checkbox:checked")
  ).map((cb) => cb.value);
  const facilities = Array.from(
    document.querySelectorAll(".owner-facility-checkbox:checked")
  ).map((cb) => cb.value);
