...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function displayReviews() {
    reviewCardsContainer.innerHTML =
      '<p class="loading-message">Loading reviews...</p>'; // Show loading message
    ...
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        reviewCard.innerHTML = `
                    <div class="rating-stars">${renderStars(
                      review.rating
                    )}</div>
                    <h3>${review.name}</h3>
      ...
      reviewCardsContainer.innerHTML =
        '<p class="form-message" style="color: red; text-align: center;">Error loading reviews. Please try again later.</p>';
    }
  }
